...
func _on_body_entered(body):
	...
	if body.is_in_group("player") or body.name == "Knight":
		print("Player collected chemical: ", ChemicalType.keys()[chemical_type])
		
		# Emit signal for chemical collection
		SignalBus.chemical_collected.emit(chemical_type, global_position)
		...
		# Queue free immediately since we don't have audio to wait for
		queue_free()
